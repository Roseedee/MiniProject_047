...
<body>
    <div class="container mt-3 align-items-center">
        ...
        <div class="container mt-5">
            <h1 class="text-center alert alert-secondary">Employee</h1>
            <?php
                if(isset($_GET['id'])) {
                    echo '<form action="update.php?form=employee" method="POST">';
                }else {
                    echo '<form action="insert.php?form=employee" method="POST">';
                }
            ?>
                ...
                <?php
                    if(isset($_GET['id'])) {
                        echo '<a href="employee.php" role="button" class="btn btn-secondary mt-3">Cancel</a>';
                        unset($_GET['id']);
                    }
                ?>
            </form>
                ...
                <?php
                    ...
                    if($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            echo "<tr>";
                                echo '<td class="text-center">'. $row['e_id'] . '</td>';
                                echo '<td class="text-center">'. $row['e_name'] . '</td>';
                                echo '<td class="text-center">'. $row['e_address'] . '</td>';
                                echo '<td class="text-center">'. $row['e_tel'] . '</td>';
                                echo '<td class="text-center"><a role="button" class="btn btn-primary" href="employee.php?id=' . $row['e_id'] .'&name='.$row['e_name'].'&address='.$row['e_address'].'&tel='.$row['e_tel'].'">Update</a></td>';
                                echo '<td class="text-center"><a role="button" class="btn btn-danger" href="delete.php?id=' . $row['e_id'] .'&form=employee">Delete</a></td>';
                            echo '</tr>';
                        }
                    }else {
